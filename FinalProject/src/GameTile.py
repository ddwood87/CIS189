...
class GameTile(tk.Frame, tk.Label):
    value: int
    key: tuple[int]
    exposed: bool
    flagged: bool
    ...
    def __init__(self, master, mine: bool = False):
        super().__init__(master, width = 20, height = 20, relief=tk.RAISED, bd=1)        
        ...
        self.image_label = tk.Label(self)#, text = self.value
        self.bind_events()
    ...
    '''def press_leave(self, event: tk.Event):    
        if self.master.current_press == self.key:
            self.unpress(self)'''
    ...
    '''def update(self):
        #debug
        self.image_label.config(text = self.value)'''
